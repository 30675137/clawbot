...
import type { clawbotApp } from "./app";
...
export async function refreshActiveTab(host: SettingsHost) {
  ...
  if (host.tab === "instances") await loadPresence(host as unknown as clawbotApp);
  if (host.tab === "sessions") await loadSessions(host as unknown as clawbotApp);
  if (host.tab === "cron") await loadCron(host);
  if (host.tab === "skills") await loadSkills(host as unknown as clawbotApp);
  if (host.tab === "nodes") {
    await loadNodes(host as unknown as clawbotApp);
    await loadDevices(host as unknown as clawbotApp);
    await loadConfig(host as unknown as clawbotApp);
    await loadExecApprovals(host as unknown as clawbotApp);
  }
  if (host.tab === "chat") {
  ...
  if (host.tab === "config") {
    await loadConfigSchema(host as unknown as clawbotApp);
    await loadConfig(host as unknown as clawbotApp);
  }
  if (host.tab === "debug") {
    await loadDebug(host as unknown as clawbotApp);
    host.eventLog = host.eventLogBuffer;
  }
  if (host.tab === "logs") {
    host.logsAtBottom = true;
    await loadLogs(host as unknown as clawbotApp, { reset: true });
    scheduleLogsScroll(
...
export async function loadOverview(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as clawbotApp, false),
    loadPresence(host as unknown as clawbotApp),
    loadSessions(host as unknown as clawbotApp),
    loadCronStatus(host as unknown as clawbotApp),
    loadDebug(host as unknown as clawbotApp),
  ]);
}

export async function loadChannelsTab(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as clawbotApp, true),
    loadConfigSchema(host as unknown as clawbotApp),
    loadConfig(host as unknown as clawbotApp),
  ]);
}
...
export async function loadCron(host: SettingsHost) {
  await Promise.all([
    loadChannels(host as unknown as clawbotApp, false),
    loadCronStatus(host as unknown as clawbotApp),
    loadCronJobs(host as unknown as clawbotApp),
  ]);
}
