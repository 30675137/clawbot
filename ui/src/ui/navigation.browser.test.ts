...
import { clawbotApp } from "./app";
import "../styles.css";

const originalConnect = clawbotApp.prototype.connect;

function mountApp(pathname: string) {
  window.history.replaceState({}, "", pathname);
  const app = document.createElement("clawbot-app") as clawbotApp;
...
afterEach(() => {
  clawbotApp.prototype.connect = originalConnect;
  window.__CLAWDBOT_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
  document.body.innerHTML = "";
});
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/clawbot/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/clawbot");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/clawbot/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__CLAWDBOT_CONTROL_UI_BASE_PATH__ = "/clawbot";
    const app = mountApp("/clawbot/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/clawbot");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/clawbot/sessions");
  });
