...
import { scheduleGatewaySigusr1Restart, triggerclawbotRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting clawbot in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerclawbotRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting clawbot via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
