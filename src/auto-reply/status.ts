...
type AgentConfig = Partial<NonNullable<NonNullable<clawbotConfig["agents"]>["defaults"]>>;
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 clawbot ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: clawbotConfig): string {
  const lines = ["ℹ️ Help", ""];
