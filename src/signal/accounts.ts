...
export function listSignalAccountIds(cfg: clawbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: clawbotConfig): string {
...
function mergeSignalAccountConfig(cfg: clawbotConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: clawbotConfig): ResolvedSignalAccount[] {
