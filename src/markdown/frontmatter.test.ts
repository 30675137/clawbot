...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { clawbot?: { emoji?: string } };
    expect(parsed.clawbot?.emoji).toBe("disk");
  });
...
`;
    ...
    const parsed = JSON5.parse(result.metadata ?? "") as { clawbot?: { events?: string[] } };
