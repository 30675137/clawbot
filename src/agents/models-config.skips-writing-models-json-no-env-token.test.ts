...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "clawbot-models-" });
}

const MODELS_CONFIG: clawbotConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureclawbotModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureclawbotModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureclawbotModelsJson } = await import("./models-config.js");
      const { resolveclawbotAgentDir } = await import("./agent-paths.js");

      await ensureclawbotModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveclawbotAgentDir(), "models.json");
