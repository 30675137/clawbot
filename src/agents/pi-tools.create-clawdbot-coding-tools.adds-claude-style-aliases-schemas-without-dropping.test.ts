...
import { createclawbotTools } from "./clawbot-tools.js";
import { __testing, createclawbotCodingTools } from "./pi-tools.js";
...
describe("createclawbotCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createclawbotTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createclawbotCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "clawbot-outside.txt");
