...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "clawbot.sandbox=1");
  args.push("--label", `clawbot.sessionKey=${params.scopeKey}`);
  args.push("--label", `clawbot.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `clawbot.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "clawbot.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`clawbot sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`clawbot sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("clawbot sandbox recreate --all");
}
