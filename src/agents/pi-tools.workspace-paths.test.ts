...
import { createclawbotCodingTools } from "./pi-tools.js";
...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("clawbot-ws-", async (workspaceDir) => {
      await withTempDir("clawbot-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createclawbotCodingTools({ workspaceDir });
    ...
    await withTempDir("clawbot-ws-", async (workspaceDir) => {
      await withTempDir("clawbot-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello clawbot");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("clawbot-ws-", async (workspaceDir) => {
      await withTempDir("clawbot-override-", async (overrideDir) => {
        const tools = createclawbotCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("clawbot-sandbox-", async (sandboxDir) => {
      await withTempDir("clawbot-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "clawbot-sbx-test",
        ...
        const tools = createclawbotCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
