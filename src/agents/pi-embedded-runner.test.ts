...
beforeAll(async () => {
  ...
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-embedded-agent-"));
...
const ensureModels = (cfg: clawbotConfig) => ensureclawbotModelsJson(cfg, agentDir) as unknown;
