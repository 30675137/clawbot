...
const defaultTools = createclawbotCodingTools();

describe("createclawbotCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createclawbotCodingTools();
    ...
    try {
      ...
      const contents = "Hello from clawbot read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "clawbot-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "clawbot-workspace"),
    ...
    const tools = createclawbotCodingTools({ sandbox });
