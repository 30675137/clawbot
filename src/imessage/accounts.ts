...
export function listIMessageAccountIds(cfg: clawbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: clawbotConfig): string {
...
function mergeIMessageAccountConfig(cfg: clawbotConfig, accountId: string): IMessageAccountConfig {
...
export function listEnabledIMessageAccounts(cfg: clawbotConfig): ResolvedIMessageAccount[] {
