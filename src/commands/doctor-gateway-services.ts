...
export async function maybeMigrateLegacyGatewayService(
  ...
  const migrate = await prompter.confirmSkipInNonInteractive({
    message: "Migrate legacy gateway services to clawbot now?",
    initialValue: true,
  });
  if (!migrate) return;
  ...
  if (loaded) {
    note(`clawbot ${service.label} already ${service.loadedText}.`, "Gateway");
    return;
  }

  const install = await prompter.confirmSkipInNonInteractive({
    message: "Install clawbot gateway service now?",
    initialValue: true,
  });
  if (!install) return;
