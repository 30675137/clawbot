...
beforeEach(() => {
  ...
  resolveclawbotPackageRoot.mockReset().mockResolvedValue(null);
  ...
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "clawbot-doctor-state-"));
...
const resolveclawbotPackageRoot = vi.fn().mockResolvedValue(null);
...
vi.mock("../plugins/loader.js", () => ({
  loadclawbotPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
vi.mock("../infra/clawbot-root.js", () => ({
  resolveclawbotPackageRoot,
}));
...
describe("doctor command", () => {
  ...
  it("warns when extra workspace directories exist", async () => {
    ...
    const legacyPath = path.join("/Users/steipete", "clawbot");
    const legacyAgentsPath = path.join(legacyPath, "AGENTS.md");
    const existsSpy = vi.spyOn(fs, "existsSync").mockImplementation((value) => {
      if (value === "/Users/steipete/clawbot" || value === legacyPath || value === legacyAgentsPath)
        return true;
    ...
    expect(
      note.mock.calls.some(
        ([message, title]) =>
          title === "Extra workspace" && typeof message === "string" && message.includes("clawbot"),
      ),
    ).toBe(true);
