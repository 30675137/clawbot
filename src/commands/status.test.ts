...
vi.mock("../infra/clawbot-root.js", () => ({
  resolveclawbotPackageRoot: vi.fn().mockResolvedValue("/tmp/clawbot"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/clawbot/pnpm-lock.yaml",
      markerPath: "/tmp/clawbot/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("clawbot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("clawbot status --all") ||
          l.includes("clawbot --profile isolated status --all") ||
          l.includes("clawbot status --all") ||
          l.includes("clawbot --profile isolated status --all"),
      ),
    ).toBe(true);
