...
import type { clawbotConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: clawbotConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("clawbot security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("clawbot configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("clawbot config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("clawbot doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("clawbot config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
