...
function resolveDefaultAccountId(cfg: clawbotConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: clawbotConfig): boolean {
...
export function summarizeBindings(cfg: clawbotConfig, bindings: AgentBinding[]): string[] {
