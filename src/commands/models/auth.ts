...
import { readConfigFileSnapshot, type clawbotConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: clawbotConfig, model: string): clawbotConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("clawbot plugins install")}\`.`,
    );
  }
