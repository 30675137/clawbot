...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveclawbotAgentDir: vi.fn().mockReturnValue("/tmp/clawbot-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveclawbotAgentDir: mocks.resolveclawbotAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/clawbot-agent/auth-profiles.json");
