...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("clawbot models auth setup-token")}\` or \`${formatCliCommand("clawbot configure")}\`.`
          : `Run \`${formatCliCommand("clawbot configure")}\` or set an API key env var.`;
