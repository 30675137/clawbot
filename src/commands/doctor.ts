...
function resolveMode(cfg: clawbotConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("clawbot doctor");

  const root = await resolveclawbotPackageRoot({
  ...
  let cfg: clawbotConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("clawbot configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("clawbot config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("clawbot setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("clawbot doctor --fix")}" to apply changes.`);
  }
