...
export async function statusCommand(
  ...
  runtime.log(theme.heading("clawbot status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("clawbot security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("clawbot security audit --deep")}`));
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("clawbot status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("clawbot logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("clawbot status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("clawbot gateway probe")}`);
  }
}
