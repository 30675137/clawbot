...
function stripUnknownConfigKeys(config: clawbotConfig): {
  config: clawbotConfig;
  removed: string[];
} {
  const parsed = clawbotSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as clawbotConfig;
...
function noteOpencodeProviderOverrides(cfg: clawbotConfig) {
...
function hasExplicitConfigPath(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.clawbot_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim());
}
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (!hasExplicitConfigPath(process.env) && snapshot.exists) {
    ...
    if (basename === "clawdbot.json") {
      const canonicalPath = path.join(path.dirname(snapshot.path), "clawbot.json");
  ...
  let cfg: clawbotConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as clawbotConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("clawbot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("clawbot doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "clawbot doctor --fix" to remove these keys.');
    }
  }
