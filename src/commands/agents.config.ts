...
export function listAgentEntries(cfg: clawbotConfig): AgentEntry[] {
...
function resolveAgentName(cfg: clawbotConfig, agentId: string) {
...
function resolveAgentModel(cfg: clawbotConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: clawbotConfig): AgentSummary[] {
