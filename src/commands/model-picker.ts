...
function resolveConfiguredModelRaw(cfg: clawbotConfig): string {
  const raw = cfg.agents?.defaults?.model as { primary?: string } | string | undefined;
  if (typeof raw === "string") return raw.trim();
  return raw?.primary?.trim() ?? "";
}

function resolveConfiguredModelKeys(cfg: clawbotConfig): string[] {
...
export function applyPrimaryModel(cfg: clawbotConfig, model: string): clawbotConfig {
...
export function applyModelAllowlist(cfg: clawbotConfig, models: string[]): clawbotConfig {
  const defaults = cfg.agents?.defaults;
