...
import { resolveclawbotAgentDir } from "../agents/agent-paths.js";
...
import { ensureclawbotModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { clawbotConfig, ModelProviderConfig } from "../config/types.js";
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<clawbotConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  ...
  const hostAgentDir = resolveclawbotAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-live-state-"));
  ...
  const agentDir = resolveclawbotAgentDir();
  const sanitizedCfg: clawbotConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-live-"));
  const tempConfigPath = path.join(tempDir, "clawbot.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.CLAWDBOT_CONFIG_PATH = tempConfigPath;

  await ensureclawbotModelsJson(nextCfg);
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "clawbot live tool probe (local, safe): " +
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureclawbotModelsJson(cfg);
