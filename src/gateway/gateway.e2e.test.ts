...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-gw-mock-home-"));
      ...
      const configPath = path.join(configDir, "clawbot.json");
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-wizard-home-"));
    process.env.HOME = tempHome;
