...
export function resolveHooksConfig(cfg: clawbotConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-clawbot-token"] === "string" ? req.headers["x-clawbot-token"].trim() : "";
