...
import { createclawbotTools } from "../agents/clawbot-tools.js";
import {
  filterToolsByPolicy,
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(getHeader(req, "x-clawbot-message-channel") ?? "");
  const accountId = getHeader(req, "x-clawbot-account-id")?.trim() || undefined;
  ...
  const allTools = createclawbotTools({
