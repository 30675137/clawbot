...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "clawbot";
  if (/clawbot/i.test(trimmed)) return trimmed;
  return `${trimmed} (clawbot)`;
}
...
export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  ...
  const siblingCli = path.join(execDir, "clawbot");
  ...
  const binCli = path.join(cwd, "bin", "clawbot.js");
