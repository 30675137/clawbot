...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `clawbot-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.CLAWDBOT_CONFIG_PATH = path.join(root, "clawbot.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "clawbot.json");
