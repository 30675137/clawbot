...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.clawbotCanvasA2UIAction ||
        window.webkit.messageHandlers.clawdbotCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.clawbotCanvasA2UIAction &&
        typeof window.clawbotCanvasA2UIAction.postMessage === "function") ||
  ...
  if (!window.clawbotSendUserAction && legacySend) {
    window.clawbotSendUserAction = legacySend;
  }
  if (!window.clawdbotSendUserAction && typeof window.clawbotSendUserAction === "function") {
    window.clawdbotSendUserAction = window.clawbotSendUserAction;
  }
  const hasHelper = () =>
    typeof window.clawbotSendUserAction === "function" ||
  ...
  window.addEventListener("clawbot:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android clawbot node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.clawbotSendUserAction === "function"
        ? window.clawbotSendUserAction
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>clawbot Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
