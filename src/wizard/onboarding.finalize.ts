...
type FinalizeOnboardingOptions = {
  flow: WizardFlow;
  opts: OnboardOptions;
  baseConfig: clawbotConfig;
  nextConfig: clawbotConfig;
  workspaceDir: string;
...
export async function finalizeOnboardingWizard(options: FinalizeOnboardingOptions) {
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.clawdbot/clawbot.json (gateway.auth.token) or CLAWDBOT_GATEWAY_TOKEN.",
        "Web UI stores a copy in this browser's localStorage (clawbot.control.settings.v1).",
        `Get the tokenized link anytime: ${formatCliCommand("clawbot dashboard --no-open")}`,
      ].join("\n"),
      "Token",
    );
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control clawbot."
            : "Copy/paste this URL in a browser on this machine to control clawbot.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
      ...
      await prompter.note(
        `When you're ready: ${formatCliCommand("clawbot dashboard --no-open")}`,
        "Later",
      );
    }
  } else if (opts.skipUi) {
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "clawbot uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("clawbot configure --section web")}`,
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened with your token; keep that tab to control clawbot."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the tokenized link above."
        : "Onboarding complete. Use the tokenized dashboard link above to control clawbot.",
  );
}
