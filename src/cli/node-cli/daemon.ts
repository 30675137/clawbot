...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("clawbot node install"), formatCliCommand("clawbot node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("clawbot node install --force")}`);
    }
    return;
  }
