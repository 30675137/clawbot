...
export function registerDnsCli(program: Command) {
  ...
  dns
    .command("setup")
    .description("Set up CoreDNS to serve clawbot.internal for unicast DNS-SD (Wide-Area Bonjour)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.clawdbot/clawbot.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      defaultRuntime.log(theme.muted("- Restrict to domain (Split DNS): clawbot.internal"));
      ...
      const serverPath = path.join(confDir, "clawbot.internal.server");
      ...
      if (zoneFileNeedsBootstrap(zonePath)) {
        ...
        const zoneLines = [
          `; created by clawbot dns setup (will be overwritten by the gateway when wide-area discovery is enabled)`,
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.clawdbot/clawbot.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
