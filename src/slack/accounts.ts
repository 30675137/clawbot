...
export function listSlackAccountIds(cfg: clawbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: clawbotConfig): string {
...
function mergeSlackAccountConfig(cfg: clawbotConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: clawbotConfig): ResolvedSlackAccount[] {
