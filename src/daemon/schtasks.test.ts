...
describe("resolveTaskScriptPath", () => {
  ...
  it("prefers CLAWDBOT_STATE_DIR over profile-derived defaults", () => {
    const env = {
      ...
      CLAWDBOT_STATE_DIR: "C:\\State\\clawbot",
    };
    expect(resolveTaskScriptPath(env)).toBe(path.join("C:\\State\\clawbot", "gateway.cmd"));
  });
...
describe("readScheduledTaskCommand", () => {
  it("parses basic command script", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-schtasks-test-"));
    try {
  ...
  it("parses script with working directory", async () => {
    ...
    try {
      ...
      await fs.writeFile(
        scriptPath,
        ["@echo off", "cd /d C:\\Projects\\clawbot", "node gateway.js"].join("\r\n"),
        "utf8",
      );
      ...
      expect(result).toEqual({
        ...
        workingDirectory: "C:\\Projects\\clawbot",
      });
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
