...
`;
    ...
    expect(parsed.clawbot.emoji).toBe("💾");
    expect(parsed.clawbot.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata as string);
    expect(parsed.clawbot.emoji).toBe("📝");
    expect(parsed.clawbot.events).toEqual(["command"]);
    expect(parsed.clawbot.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.clawbot.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"clawbot": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"clawbot": {"events": ["test"]}}');
  });
...
describe("resolveclawbotMetadata", () => {
  it("extracts clawbot metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveclawbotMetadata(frontmatter);
  ...
  it("returns undefined when clawbot key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        clawbot: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with clawbot" },
            { id: "npm", kind: "npm", package: "@clawbot/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@clawbot/hook");
  });
    ...
    "clawbot":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with clawbot" }],
      },
  }
---
...
`;
    ...
    const clawbot = resolveclawbotMetadata(frontmatter);
    expect(clawbot).toBeDefined();
    expect(clawbot?.emoji).toBe("💾");
    expect(clawbot?.events).toEqual(["command:new"]);
    expect(clawbot?.requires?.config).toEqual(["workspace.dir"]);
    expect(clawbot?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const clawbot = resolveclawbotMetadata(frontmatter);
    expect(clawbot?.emoji).toBe("disk");
    expect(clawbot?.events).toEqual(["command:new"]);
  });
});
