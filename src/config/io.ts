...
import type { clawbotConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import { validateConfigObjectWithPlugins } from "./validation.js";
import { compareclawbotVersions } from "./version.js";
...
function coerceConfig(value: unknown): clawbotConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as clawbotConfig;
}
...
function stampConfigVersion(cfg: clawbotConfig): clawbotConfig {
...
function warnIfConfigFromFuture(cfg: clawbotConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) return;
  const cmp = compareclawbotVersions(VERSION, touched);
  if (cmp === null) return;
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer clawbot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: clawbotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): clawbotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as clawbotConfig, deps.env);
      }
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as clawbotConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: clawbotConfig) {
    clearConfigCache();
...
export function loadConfig(): clawbotConfig {
...
export async function writeConfigFile(cfg: clawbotConfig): Promise<void> {
  clearConfigCache();
