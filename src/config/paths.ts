...
import type { clawbotConfig } from "./types.js";
...
const NEW_STATE_DIRNAME = ".clawbot";
const CONFIG_FILENAME = "clawbot.json";
 ...
 * Can be overridden via clawbot_STATE_DIR (preferred) or CLAWDBOT_STATE_DIR (legacy).
 * Default: ~/.clawdbot (legacy default for compatibility)
 * If ~/.clawbot exists and ~/.clawdbot does not, prefer ~/.clawbot.
 */
export function resolveStateDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const override = env.clawbot_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
 ...
 * Can be overridden via clawbot_CONFIG_PATH (preferred) or CLAWDBOT_CONFIG_PATH (legacy).
 * Default: ~/.clawdbot/clawbot.json (or $*_STATE_DIR/clawbot.json)
 */
export function resolveCanonicalConfigPath(
  env: NodeJS.ProcessEnv = process.env,
  stateDir: string = resolveStateDir(env, os.homedir),
): string {
  const override = env.clawbot_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) return resolveUserPath(override);
...
export function resolveConfigPath(
  ...
  const override = env.clawbot_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) return resolveUserPath(override);
  const stateOverride = env.clawbot_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  const candidates = [
...
export function resolveDefaultConfigCandidates(
  ...
  const explicit = env.clawbot_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) return [resolveUserPath(explicit)];

  const candidates: string[] = [];
  const clawbotStateDir = env.clawbot_STATE_DIR?.trim();
  if (clawbotStateDir) {
    candidates.push(path.join(resolveUserPath(clawbotStateDir), CONFIG_FILENAME));
    candidates.push(path.join(resolveUserPath(clawbotStateDir), LEGACY_CONFIG_FILENAME));
  }
 ...
 * Default: os.tmpdir()/clawbot-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `clawbot-${uid}` : "clawbot";
