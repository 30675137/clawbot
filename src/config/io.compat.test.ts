...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "clawbot-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".clawbot" | ".clawdbot",
  port: number,
  filename: "clawbot.json" | "clawdbot.json" = "clawbot.json",
) {
...
describe("config io compat (new + legacy folders)", () => {
  it("prefers ~/.clawbot/clawbot.json when both configs exist", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".clawbot", 19001);
  ...
  it("falls back to ~/.clawdbot/clawbot.json when only legacy exists", async () => {
    await withTempHome(async (home) => {
  ...
  it("prefers clawbot.json over legacy filename in the same dir", async () => {
    await withTempHome(async (home) => {
      const preferred = await writeConfig(home, ".clawdbot", 20003, "clawbot.json");
  ...
  it("honors explicit legacy config path env override", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".clawbot", 19002);
