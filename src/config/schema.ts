...
export type ConfigSchema = ReturnType<typeof clawbotSchema.toJSONSchema>;
...
const FIELD_HELP: Record<string, string> = {
  "meta.lastTouchedVersion": "Auto-set when clawbot writes the config.",
  ...
  "gateway.controlUi.basePath":
    "Optional URL prefix where the Control UI is served (e.g. /clawbot).",
  ...
  "plugins.installs":
    "CLI-managed install metadata (used by `clawbot plugins update` to locate install sources).",
...
const FIELD_PLACEHOLDERS: Record<string, string> = {
  ...
  "gateway.controlUi.basePath": "/clawbot",
...
function buildBaseConfigSchema(): ConfigSchemaResponse {
  ...
  const schema = clawbotSchema.toJSONSchema({
    target: "draft-07",
    unrepresentable: "any",
  });
  schema.title = "clawbotConfig";
