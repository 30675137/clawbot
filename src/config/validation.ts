...
import type { clawbotConfig, ConfigValidationIssue } from "./types.js";
import { clawbotSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: clawbotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: clawbotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = clawbotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as clawbotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as clawbotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as clawbotConfig)),
    ),
  };
}
