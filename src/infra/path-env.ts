...
function candidateBinDirs(opts: EnsureclawbotPathOpts): string[] {
  ...
  // Bundled macOS app: `clawbot` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingclawbot = path.join(execDir, "clawbot");
    if (isExecutable(siblingclawbot)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/clawbot` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "clawbot"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `clawbot` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureclawbotCliOnPath(opts: EnsureclawbotPathOpts = {}) {
