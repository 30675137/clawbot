...
export const WIDE_AREA_DISCOVERY_DOMAIN = "clawbot.internal.";
export const WIDE_AREA_ZONE_FILENAME = "clawbot.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*clawbot-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "clawbot";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "clawbot");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "clawbot-gw");
  ...
  records.push(`_clawbot-gw._tcp IN PTR ${instanceLabel}._clawbot-gw._tcp`);
  records.push(`${instanceLabel}._clawbot-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._clawbot-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; clawbot-content-hash: ${contentHash}\n${contentBody}`;
}
