...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `clawbot-plugins-${randomUUID()}`);
...
describe("discoverclawbotPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverclawbotPlugins } = await import("./discovery.js");
      return discoverclawbotPlugins({ workspaceDir });
    });
  ...
  it("loads package extension packs", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "pack",
        clawbot: { extensions: ["./src/one.ts", "./src/two.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("derives unscoped ids for scoped packages", async () => {
    ...
    fs.writeFileSync(
      path.join(globalExt, "package.json"),
      JSON.stringify({
        name: "@clawbot/voice-call",
        clawbot: { extensions: ["./src/index.ts"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@clawbot/demo-plugin-dir",
        clawbot: { extensions: ["./index.js"] },
      }),
      "utf-8",
    );
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      const { discoverclawbotPlugins } = await import("./discovery.js");
      return discoverclawbotPlugins({ extraPaths: [packDir] });
    });
