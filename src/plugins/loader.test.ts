...
import { loadclawbotPlugins } from "./loader.js";
...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `clawbot-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "clawbot.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
describe("loadclawbotPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadclawbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadclawbotPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("preserves package.json metadata for bundled memory plugins", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@clawbot/memory-core",
        version: "1.2.3",
        description: "Memory plugin package",
        clawbot: { extensions: ["./index.ts"] },
      }),
      "utf-8",
    );
    writePlugin({
