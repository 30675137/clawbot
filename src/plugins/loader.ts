...
import { discoverclawbotPlugins } from "./discovery.js";
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: clawbotPluginDefinition;
  register?: clawbotPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as clawbotPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as clawbotPluginDefinition;
...
export function loadclawbotPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverclawbotPlugins({
    workspaceDir: options.workspaceDir,
    ...
    ...(pluginSdkAlias
      ? {
          alias: {
            ...
            "clawbot/plugin-sdk": pluginSdkAlias,
          },
        }
      : {}),
  });
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: clawbotPluginModule | null = null;
    try {
      mod = jiti(candidate.source) as clawbotPluginModule;
    } catch (err) {
