...
function setWhatsAppDmPolicy(cfg: clawbotConfig, dmPolicy: DmPolicy): clawbotConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: clawbotConfig, allowFrom?: string[]): clawbotConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: clawbotConfig, selfChatMode: boolean): clawbotConfig {
...
async function detectWhatsAppLinked(cfg: clawbotConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppAllowFrom(
  ...
  if (options?.forceAllowlist) {
    await prompter.note(
      "We need the sender/owner number so clawbot can allowlist you.",
      "WhatsApp number",
    );
  ...
  const phoneMode = (await prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for clawbot" },
    ],
...
export const whatsappOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("clawbot channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
