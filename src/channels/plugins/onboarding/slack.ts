...
function setSlackDmPolicy(cfg: clawbotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "clawbot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for clawbot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: clawbotConfig, allowFrom: string[]): clawbotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
