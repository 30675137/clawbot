...
import { discoverclawbotPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { clawbotPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  clawbot?: clawbotPackageManifest;
  [LEGACY_MANIFEST_KEY]?: clawbotPackageManifest;
};
...
function toChannelMeta(params: {
  channel: NonNullable<clawbotPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: clawbotPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageclawbot?: clawbotPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageclawbot;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.clawbot ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageclawbot: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverclawbotPlugins({ workspaceDir: options.workspaceDir });
