...
IMAGE_NAME="${CLAWDBOT_IMAGE:-clawbot:local}"
...
echo ""
docker compose "${COMPOSE_ARGS[@]}" run --rm clawbot-cli onboard --no-install-daemon

echo ""
echo "==> Provider setup (optional)"
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm clawbot-cli providers login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm clawbot-cli providers add --provider telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm clawbot-cli providers add --provider discord --token <token>"
echo "Docs: https://docs.molt.bot/providers"

echo ""
echo "==> Starting gateway"
docker compose "${COMPOSE_ARGS[@]}" up -d clawbot-gateway
...
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f clawbot-gateway"
echo "  ${COMPOSE_HINT} exec clawbot-gateway node dist/index.js health --token \"$CLAWDBOT_GATEWAY_TOKEN\""
