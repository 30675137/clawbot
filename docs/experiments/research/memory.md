...
read_when:
  ...
  - Deciding: standalone CLI vs deep clawbot integration
...
Recommendation: **deep integration in clawbot**, but keep a separable core library.

### Why integrate into clawbot?
- clawbot already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `clawbot memory recall "…" --k 25 --since 30d`
  - `clawbot memory reflect --since 7d`
