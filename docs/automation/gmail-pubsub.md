---
summary: "Gmail Pub/Sub push wired into clawbot webhooks via gogcli"
read_when:
  - Wiring Gmail inbox triggers to clawbot
  - Setting up Pub/Sub push for agent wake
---

# Gmail Pub/Sub -> clawbot

Goal: Gmail watch -> Pub/Sub push -> `gog gmail watch serve` -> clawbot webhook.

## Prereqs

- `gcloud` installed and logged in ([install guide](https://docs.cloud.google.com/sdk/docs/install-sdk)).
- `gog` (gogcli) installed and authorized for the Gmail account ([gogcli.sh](https://gogcli.sh/)).
- clawbot hooks enabled (see [Webhooks](/automation/webhook)).
...
Use the clawbot helper to wire everything together (installs deps on macOS via brew):

```bash
clawbot webhooks gmail setup \
  --account clawbot@gmail.com
```

Defaults:
- Uses Tailscale Funnel for the public push endpoint.
- Writes `hooks.gmail` config for `clawbot webhooks gmail run`.
- Enables the Gmail hook preset (`hooks.presets: ["gmail"]`).

Path note: when `tailscale.mode` is enabled, clawbot automatically sets
...
- `--hook-url` points to clawbot `/hooks/gmail` (mapped; isolated run + summary to main).
- `--include-body` and `--max-bytes` control the body snippet sent to clawbot.

Recommended: `clawbot webhooks gmail run` wraps the same flow and auto-renews the watch.
...
```bash
gog gmail watch status --account clawbot@gmail.com
gog gmail history --account clawbot@gmail.com --since <historyId>
```
...
```bash
gog gmail watch stop --account clawbot@gmail.com
