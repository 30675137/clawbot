---
summary: "Use Synthetic's Anthropic-compatible API in clawbot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. clawbot registers it as the
...
```bash
clawbot onboard --auth-choice synthetic-api-key
```
...
Note: clawbot's Anthropic client appends `/v1` to the base URL, so use
