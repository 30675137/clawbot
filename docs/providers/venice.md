---
summary: "Use Venice AI privacy-focused models in clawbot"
read_when:
  - You want privacy-focused inference in clawbot
...
```bash
clawbot onboard --auth-choice venice-api-key
```
...
```bash
clawbot onboard --non-interactive \
...
```bash
clawbot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, clawbot shows all available Venice models. Pick based on your needs:
...
```bash
clawbot models set venice/claude-opus-45
clawbot models set venice/llama-3.3-70b
```

List all available models:

```bash
clawbot models list | grep venice
```

## Configure via `clawbot configure`

1. Run `clawbot configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
clawbot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
clawbot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
clawbot chat --model venice/claude-opus-45

# Use uncensored model
clawbot chat --model venice/venice-uncensored

# Use vision model with image
clawbot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
clawbot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `clawbot models list` to see currently available models. Some models may be temporarily offline.
