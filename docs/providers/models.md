---
summary: "Model providers (LLMs) supported by clawbot"
read_when:
...
clawbot can use many LLM providers. Pick one, authenticate, then set the default
...
1) Authenticate with the provider (usually via `clawbot onboard`).
