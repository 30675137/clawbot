---
summary: "Direct `clawbot agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
---
# `clawbot agent` (direct agent runs)

`clawbot agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `clawbot message --target`).
...
```bash
clawbot agent --to +15555550123 --message "status update"
...
clawbot agent --to +15555550123 --message "Summon reply" --deliver
