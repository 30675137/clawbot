---
summary: "Chrome extension: let clawbot drive your existing Chrome tab"
read_when:
...
The clawbot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
clawbot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the clawbot release (npm package) as static files. There is no separate “build” step.

After upgrading clawbot:
- Re-run `clawbot browser extension install` to refresh the installed files under your clawbot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

clawbot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want clawbot to control.
...
- `ON`: attached; clawbot can drive that tab.
...
Debugging: `clawbot sandbox explain`
...
`clawbot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `clawbot browser extension install` first to copy the extension to a stable location under your clawbot state directory.
