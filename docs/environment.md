---
summary: "Where clawbot loads environment variables and the precedence order"
read_when:
...
clawbot pulls environment variables from multiple sources. The rule is **never override existing values**.
...
4) **Config `env` block** in `~/.clawdbot/clawbot.json` (applied only if missing).
