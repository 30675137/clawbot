...
- `/usage cost` in chats: local cost summary aggregated from clawbot session logs.
- CLI: `clawbot status --usage` prints a full per-provider breakdown.
- CLI: `clawbot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
