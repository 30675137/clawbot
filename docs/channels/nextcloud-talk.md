...
```bash
clawbot plugins install @clawbot/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
clawbot plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "clawbot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawbot pairing list nextcloud-talk`
  - `clawbot pairing approve nextcloud-talk <CODE>`
