...
Matrix is an open, decentralized messaging protocol. clawbot connects as a Matrix **user**
...
```bash
clawbot plugins install @clawbot/matrix
```

Local checkout (when running from a git repo):

```bash
clawbot plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `clawbot plugins install @clawbot/matrix`
   - From a local checkout: `clawbot plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: clawbot calls the same
...
- On first connection, clawbot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  clawbot logs a warning.
...
- Approve via:
  - `clawbot pairing list matrix`
  - `clawbot pairing approve matrix <CODE>`
...
- On startup, clawbot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
