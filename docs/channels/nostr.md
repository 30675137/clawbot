...
read_when:
  - You want clawbot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables clawbot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`clawbot onboard`) and `clawbot channels add` list optional channel plugins.
...
```bash
clawbot plugins install @clawbot/nostr
```

Use a local checkout (dev workflows):

```bash
clawbot plugins install --link <path-to-clawbot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "clawbot@example.com",
        "lud16": "clawbot@example.com"
      }
    }
  }
}
