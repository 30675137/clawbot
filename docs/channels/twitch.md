...
read_when:
  - Setting up Twitch chat integration for clawbot
---
# Twitch (plugin)

Twitch chat support via IRC connection. clawbot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
clawbot plugins install @clawbot/twitch
```

Local checkout (when running from a git repo):

```bash
clawbot plugins install ./extensions/twitch
```
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "clawbot",              // Bot's Twitch account
