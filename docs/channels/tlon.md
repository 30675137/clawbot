...
Tlon is a decentralized messenger built on Urbit. clawbot connects to your Urbit ship and can
...
```bash
clawbot plugins install @clawbot/tlon
```

Local checkout (when running from a git repo):

```bash
clawbot plugins install ./extensions/tlon
```
...
Use these with `clawbot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, clawbot replies in-thread.
