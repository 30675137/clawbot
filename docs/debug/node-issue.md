...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in clawbot
---

# Node + tsx "__name is not a function" crash

## Summary
Running clawbot via Node with `tsx` fails at startup with:

```
[clawbot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawbot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch clawbot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node clawbot.mjs status` works on Node 25.
