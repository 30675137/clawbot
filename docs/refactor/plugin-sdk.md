...
Accessed via `clawbotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: clawbotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: clawbotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: clawbotConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: clawbotConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: clawbotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: clawbotConfig): string;
  };
};
```
...
- Add `api.runtime` to `clawbotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawbotRuntime: ">=2026.2.0"`).
