...
Remote Gateway:
```bash
clawbot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `clawbot tui --deliver`
...
- Check the Gateway logs: `clawbot logs --follow`.
- Confirm the agent can run: `clawbot status` and `clawbot models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `clawbot agents list` and your routing config.
