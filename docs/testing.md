...
clawbot has three Vitest suites (unit/integration, e2e, live) and a small set of Docker runners.
...
### Layer 2: Gateway + dev agent smoke (what “@clawbot” actually does)
...
```bash
clawbot models auth paste-token --provider anthropic --profile-id anthropic:setup-token-test
...
- Gemini API vs Gemini CLI:
  - API: clawbot calls Google’s hosted Gemini API over HTTP (API key / profile auth); this is what most users mean by “Gemini”.
  - CLI: clawbot shells out to a local `gemini` binary; it has its own auth and can behave differently (streaming/tool support/version skew).
...
- OpenRouter: `openrouter/...` (hundreds of models; use `clawbot models scan` to find tool+image capable candidates)
...
- If a live test says “no creds”, debug the same way you’d debug `clawbot models list` / model selection.

- Profile store: `~/.clawdbot/credentials/` (preferred; what “profile keys” means in the tests)
- Config: `~/.clawdbot/clawbot.json` (or `CLAWDBOT_CONFIG_PATH`)
