---
summary: "CLI reference for `clawbot doctor` (health checks + guided repairs)"
read_when:
...
- `--fix` (alias for `--repair`) writes a backup to `~/.clawdbot/clawbot.json.bak` and drops unknown config keys, listing each removal.
