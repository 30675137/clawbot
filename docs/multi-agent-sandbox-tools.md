...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `clawbot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `clawbot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
1. **Check agent resolution:**
   ```exec
   clawbot agents list --bindings
   ```

2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=clawbot.sandbox=1"
   ```
