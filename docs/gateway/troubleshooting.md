---
summary: "Quick troubleshooting guide for common clawbot failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When clawbot misbehaves, here's how to fix it.
...
|---|---|---|
| `clawbot status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `clawbot status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `clawbot status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `clawbot gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `clawbot channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `clawbot gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `clawbot logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `clawbot status --all` (it redacts tokens). If you paste `clawbot status`, consider setting `CLAWDBOT_SHOW_SECRETS=0` first (token previews).
...
```bash
clawbot models auth paste-token --provider anthropic
clawbot models status
```
...
**Logs:**
- Preferred: `clawbot logs --follow`
- File logs (always): `/tmp/clawbot/clawbot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CLAWDBOT_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u clawbot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  clawbot config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  clawbot config set gateway.mode remote
  clawbot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `clawbot setup` to create a starter config, then rerun
the gateway.
...
service was installed with Bun or a version-managed Node path, run `clawbot doctor`
...
- then run `clawbot sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `clawbot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `clawbot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `clawbot gateway status` and `clawbot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
...
**If `clawbot gateway status` shows a config mismatch**
...
- Fix: rerun `clawbot gateway install --force` from the same `--profile` / `CLAWDBOT_STATE_DIR` you want the service to use.

**If `clawbot gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `clawbot doctor` to update it (or `clawbot gateway install --force` for a full rewrite).
...
**If `clawbot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/clawbot` on disk.
...
clawbot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `clawbot models list` or
  `clawbot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdbot/clawbot.json}"
```
...
tail -f "$(ls -t /tmp/clawbot/clawbot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
clawbot logs --follow | grep "pairing request"
```
...
clawbot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/clawbot/clawbot-*.log | head -1)" | tail -20
```

### High Memory Usage

clawbot keeps conversation history in memory.
...
clawbot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `clawbot doctor` reports every invalid entry.
- `clawbot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `clawbot logs`, `clawbot health`, `clawbot status`, `clawbot gateway status`, and `clawbot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/clawbot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `clawbot models status` (CLI).
...
5) Run `clawbot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. clawbot scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/clawbot/clawbot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
Ensure the global `clawbot` CLI is installed and matches the app version:
```bash
clawbot --version
npm install -g clawbot@<version>
```
...
#   ${CLAWDBOT_CONFIG_PATH:-$HOME/.clawdbot/clawbot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
clawbot gateway --verbose
clawbot channels login --verbose
```
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/clawbot/clawbot-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CLAWDBOT_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdbot/logs/...`; profiles use `~/.clawdbot-<profile>/logs/...`)<br />Linux: `journalctl --user -u clawbot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "clawbot Gateway (<profile>)" /V /FO LIST` |
...
tail -20 /tmp/clawbot/clawbot-*.log
```
...
clawbot channels login         # re-pair WhatsApp
clawbot gateway restart           # or: clawbot gateway
```
...
1. Check logs first: `/tmp/clawbot/` (default: `clawbot-YYYY-MM-DD.log`, or your configured `logging.file`)
