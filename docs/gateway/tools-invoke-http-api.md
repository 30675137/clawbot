...
clawbot’s Gateway exposes a simple HTTP endpoint for invoking a single tool directly. It is always enabled, but gated by Gateway auth and tool policy.
...
- `x-clawbot-message-channel: <channel>` (example: `slack`, `telegram`)
- `x-clawbot-account-id: <accountId>` (when multiple accounts exist)
