...
- Replaces the legacy `gateway` command. CLI entry point: `clawbot gateway`.
...
pnpm gateway:watch
```
- Config hot reload watches `~/.clawdbot/clawbot.json` (or `CLAWDBOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__clawbot__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBOT_SKIP_CANVAS_HOST=1`.
...
- Linux: `clawbot-gateway-<profile>.service`
- Windows: `clawbot Gateway (<profile>)`

Install metadata is embedded in the service config:
- `CLAWDBOT_SERVICE_MARKER=clawbot`
...
```bash
clawbot --dev setup
clawbot --dev gateway --allow-unconfigured
...
- `CLAWDBOT_CONFIG_PATH=~/.clawdbot-dev/clawbot.json`
...
```bash
clawbot --profile main gateway install
clawbot --profile rescue gateway install
```

Example:
```bash
CLAWDBOT_CONFIG_PATH=~/.clawdbot/a.json CLAWDBOT_STATE_DIR=~/.clawdbot-a clawbot gateway --port 19001
CLAWDBOT_CONFIG_PATH=~/.clawdbot/b.json CLAWDBOT_STATE_DIR=~/.clawdbot-b clawbot gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `clawbot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `clawbot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `clawbot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.clawdbot.*` is cleaned up).
  - `clawbot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are clawbot profile services.
  ...
  - Cleanup: `clawbot gateway uninstall` (current service) and `clawbot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `clawbot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- clawbot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.clawdbot.*` labels still unload cleanly).
- To stop it cleanly, use `clawbot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `clawbot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `clawbot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)
clawbot installs a **systemd user service** by default on Linux/WSL2. We
...
`clawbot gateway install` writes the user unit. `clawbot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/clawbot-gateway[-<profile>].service`:
```
[Unit]
Description=clawbot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/clawbot gateway --port 18789
Restart=always
RestartSec=5
...
```
systemctl --user enable --now clawbot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/clawbot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now clawbot-gateway[-<profile>].service
```
...
## CLI helpers
- `clawbot gateway health|status` — request health/status over the Gateway WS.
- `clawbot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `clawbot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `clawbot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `clawbot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `clawbot gateway` and the legacy TCP control port.
