...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “clawbot runs”).
...
With the tunnel up:
- `clawbot health` and `clawbot status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `clawbot gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.
