---
summary: "All configuration options for ~/.clawdbot/clawbot.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

clawbot reads an optional **JSON5** config from `~/.clawdbot/clawbot.json` (comments + trailing commas allowed).

If the file is missing, clawbot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
clawbot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `clawbot doctor`, `clawbot logs`, `clawbot health`, `clawbot status`, `clawbot service`, `clawbot help`).
- Run `clawbot doctor` to see the exact issues.
- Run `clawbot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `clawbot config set`. Keep a backup of `~/.clawdbot/clawbot.json`.
...
```bash
clawbot gateway call config.get --params '{}' # capture payload.hash
clawbot gateway call config.apply --params '{
...
```bash
clawbot gateway call config.get --params '{}' # capture payload.hash
clawbot gateway call config.patch --params '{
...
{ "$include": "/etc/clawbot/base.json5" } // absolute
...
clawbot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, clawbot runs your login shell and imports only the missing expected keys (never overrides).
...
clawbot stores **per-agent** auth profiles (OAuth + API keys) in:
...
- `~/.clawdbot/agent/*` (migrated by `clawbot doctor` into `~/.clawdbot/agents/<defaultAgentId>/agent/*`)
...
On first use, clawbot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, clawbot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/clawbot/clawbot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/clawbot/clawbot.log`.
...
- The legacy single-account Baileys auth dir is migrated by `clawbot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "clawbot", "clawd"] } }
    ]
  }
}
```
...
- `commands.config: true` enables `/config` (reads/writes `clawbot.json`).
...
clawbot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
clawbot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `clawbot plugins install @clawbot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
clawbot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
clawbot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, clawbot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/clawbot" } }
}
```
...
When a file exceeds this limit, clawbot logs a warning and injects a truncated
...
message envelopes). If unset, clawbot uses the host timezone at runtime.
...
`[clawbot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[clawbot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[clawbot]"`, clawbot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, clawbot generates audio
...
clawbot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, clawbot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `clawbot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:clawbot`: all built-in clawbot tools (excludes provider plugins)
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        docker: {
          image: "clawbot-sandbox:bookworm-slim",
          containerPrefix: "clawbot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
          ...
          apparmorProfile: "clawbot-sandbox",
          dns: ["1.1.1.1", "8.8.8.8"],
        ...
        browser: {
          enabled: false,
          image: "clawbot-sandbox-browser:bookworm-slim",
          containerPrefix: "clawbot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
OpenCode Zen is a multi-model gateway with per-model endpoints. clawbot uses
...
- Shortcut: `clawbot onboard --auth-choice opencode-zen`.
...
Shortcut: `clawbot onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `clawbot onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `clawbot onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `clawbot onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `clawbot onboard --auth-choice minimax-api`.
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, clawbot stays in idle-only mode for backward compatibility.
...
clawbot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (clawbot.app menubar, or `clawbot gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/clawbot" }
...
- Examples: `"/ui"`, `"/clawbot"`, `"/apps/clawbot"`.
...
- When a connection comes from one of these IPs, clawbot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `clawbot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
...
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  ...
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. clawbot
...
macOS app behavior:
- clawbot.app watches `~/.clawdbot/clawbot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawdbot/clawbot.json` (or `CLAWDBOT_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.clawdbot/clawbot.json` (or `CLAWDBOT_CONFIG_PATH`)
...
- `clawbot --dev …` → uses `~/.clawdbot-dev` + shifts ports from base `19001`
- `clawbot --profile <name> …` → uses `~/.clawdbot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `clawbot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "clawbot@gmail.com",
...
Note: when `tailscale.mode` is on, clawbot defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawbot/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__clawbot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
Controls LAN mDNS discovery broadcasts (`_clawbot-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_clawbot-bridge._tcp` under `~/.clawdbot/dns/` using the standard discovery domain `clawbot.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `clawbot.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `clawbot.internal` via that server
