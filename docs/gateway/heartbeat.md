...
- During heartbeat runs, clawbot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawbot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, clawbot skips the heartbeat run entirely (no model call).
...
```bash
clawbot system event --text "Check for urgent follow-ups" --mode now
```
