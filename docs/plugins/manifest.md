...
read_when:
  - You are building a clawbot plugin
  - You need to ship a plugin config schema or debug plugin validation errors
---
# Plugin manifest (clawbot.plugin.json)

Every plugin **must** ship a `clawbot.plugin.json` file in the **plugin root**.
clawbot uses this manifest to validate configuration **without executing plugin
