---
summary: "First-run onboarding flow for clawbot (macOS app)"
read_when:
...
The app can install the global `clawbot` CLI via npm/pnpm so terminal
...
On the first agent run, clawbot bootstraps a workspace (default `~/clawd`):
...
```bash
clawbot webhooks gmail setup --account you@gmail.com
```
