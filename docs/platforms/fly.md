---
title: Fly.io
description: Deploy clawbot on Fly.io
---

# Fly.io Deployment

**Goal:** clawbot Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
fly apps create my-clawbot

# Create a persistent volume (1GB is usually enough)
fly volumes create clawbot_data --size 1 --region iad
```
...
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `clawbot.json` where they could be accidentally exposed or logged.
...
**Note:** With `CLAWDBOT_STATE_DIR=/data`, the config path is `/data/clawbot.json`.
...
Or visit `https://my-clawbot.fly.dev/`
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/clawbot.json` should be read on restart.

Verify the config exists:
```bash
fly ssh console --command "cat /data/clawbot.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/clawbot.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/clawbot.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:
```bash
fly ssh console --command "rm /data/clawbot.json"
```
...
```bash
# List current IPs
fly ips list -a my-clawbot

# Release public IPs
fly ips release <public-ipv4> -a my-clawbot
fly ips release <public-ipv6> -a my-clawbot
...
fly ips allocate-v6 --private -a my-clawbot
```
...
fly proxy 3000:3000 -a my-clawbot
