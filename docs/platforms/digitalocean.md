---
summary: "clawbot on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up clawbot on DigitalOcean
  - Looking for cheap VPS hosting for clawbot
---

# clawbot on DigitalOcean

## Goal

Run a persistent clawbot Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status clawbot-gateway.service

# View logs
journalctl --user -u clawbot-gateway.service -f
```
...
```bash
clawbot config set gateway.bind tailnet
clawbot gateway restart
```
...
### WhatsApp
```bash
clawbot channels login whatsapp
# Scan QR code
```
