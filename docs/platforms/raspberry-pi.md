---
summary: "clawbot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up clawbot on a Raspberry Pi
  - Running clawbot on ARM devices
  - Building a cheap always-on personal AI
---

# clawbot on Raspberry Pi

## Goal

Run a persistent, always-on clawbot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most clawbot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled clawbot
...
```bash
# Check logs
journalctl -u clawbot --no-pager -n 100

# Common fix: rebuild
cd ~/clawbot  # if using hackable install
npm run build
sudo systemctl restart clawbot
```
