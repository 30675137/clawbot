...
read_when:
  - "You installed clawbot but `clawbot` is “command not found”"
...
clawbot’s runtime baseline is **Node 22+**.

If you can run `npm install -g clawbot@latest` but later see `clawbot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
