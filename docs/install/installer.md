...
clawbot ships two installer scripts (served from `molt.bot`):
...
If the installer completes but `clawbot` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- Choose install method:
  - `npm` (default): `npm install -g clawbot@latest`
...
- If upgrading an existing install: runs `clawbot doctor --non-interactive` (best effort).
- For git installs: runs `clawbot doctor --non-interactive` after install/update (best effort).
...
If you run the installer while **already inside a clawbot source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:
...
This script installs `clawbot` into a prefix (default: `~/.clawdbot`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.
...
- Choose install method:
  - `npm` (default): `npm install -g clawbot@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `clawbot doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://molt.bot/install.ps1 | iex -InstallMethod git -GitDir "C:\\clawbot"
```
...
- **"clawbot" is not recognized**: your npm global bin folder is not on PATH. Most systems use
