---
summary: "Optional Docker-based setup and onboarding for clawbot"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run clawbot on a host without local installs.
- **No**: you’re running on your own machine and just want the fastest dev loop. Use the normal install flow instead.
- **Sandboxing note**: agent sandboxing uses Docker too, but it does **not** require the full gateway to run in Docker. See [Sandboxing](/gateway/sandboxing).

This guide covers:
- Containerized Gateway (full clawbot in Docker)
...
```bash
docker build -t clawbot:local -f Dockerfile .
docker compose run --rm clawbot-cli onboard
...
`clawbot-gateway` and `clawbot-cli` by generating `docker-compose.extra.yml`.
...
```bash
export CLAWDBOT_HOME_VOLUME="clawbot_home"
./docker-setup.sh
```
...
WhatsApp (QR):
```bash
docker compose run --rm clawbot-cli channels login
```

Telegram (bot token):
```bash
docker compose run --rm clawbot-cli channels add --channel telegram --token "<token>"
```

Discord (bot token):
```bash
docker compose run --rm clawbot-cli channels add --channel discord --token "<token>"
```
...
```bash
docker compose exec clawbot-gateway node dist/index.js health --token "$CLAWDBOT_GATEWAY_TOKEN"
```
...
clawbot auto-recreates containers when `setupCommand` (or docker config) changes
unless the container was **recently used** (within ~5 minutes). Hot containers
log a warning with the exact `clawbot sandbox recreate ...` command.
...
This builds `clawbot-sandbox:bookworm-slim` using `Dockerfile.sandbox`.
...
This builds `clawbot-sandbox-common:bookworm-slim`. To use it:

```json5
{
  agents: { defaults: { sandbox: { docker: { image: "clawbot-sandbox-common:bookworm-slim" } } } }
}
```
...
This builds `clawbot-sandbox-browser:bookworm-slim` using
...
```json5
{
  agents: {
    defaults: {
      sandbox: { browser: { image: "my-clawbot-browser" } }
    }
  }
}
```
...
```bash
docker build -t my-clawbot-sbx -f Dockerfile.sandbox .
```
...
```json5
{
  agents: {
    defaults: {
      sandbox: { docker: { image: "my-clawbot-sbx" } }
    }
  }
}
```
...
- Image missing: build with [`scripts/sandbox-setup.sh`](https://github.com/clawbot/clawbot/blob/main/scripts/sandbox-setup.sh) or set `agents.defaults.sandbox.docker.image`.
- Container not running: it will auto-create per session on demand.
- Permission errors in sandbox: set `docker.user` to a UID:GID that matches your
  mounted workspace ownership (or chown the workspace folder).
- Custom tools not found: clawbot runs commands with `sh -lc` (login shell), which
