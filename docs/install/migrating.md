---
summary: "Move (migrate) a clawbot install from one machine to another"
read_when:
  - You are moving clawbot to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
---
# Migrating clawbot to a new machine

This guide migrates a clawbot Gateway from one machine to another **without redoing onboarding**.
...
- Gateway configuration (`clawbot.json`)
...
cd ~
tar -czf clawbot-state.tgz .clawdbot
...
### Step 1 — Install clawbot on the new machine
...
### Footgun: copying only `clawbot.json`

`clawbot.json` is not enough. Many providers store state under:
...
- `clawbot status` shows the gateway running
...
- [Where does clawbot store its data?](/help/faq#where-does-clawbot-store-its-data)
