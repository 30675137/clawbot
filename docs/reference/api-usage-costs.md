...
clawbot features that can generate provider usage or paid API calls.
...
- `clawbot status --usage` and `clawbot channels list` show provider **usage windows**
...
clawbot can pick up credentials from:
...
- `clawbot status --usage`
- `clawbot models status --json`
...
### 8) Model scan / probe
`clawbot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
