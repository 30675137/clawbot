...
This document explains how clawbot manages sessions end-to-end:
...
clawbot is designed around a single **Gateway process** that owns session state.
...
clawbot persists sessions in two layers:
...
clawbot resolves these via `src/config/sessions.ts`.
...
clawbot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (clawbot consumes the events, but Pi decides when to compact).
...
clawbot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, clawbot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, clawbot leaves it alone.
...
- `clawbot status` (CLI)
- `clawbot sessions` / `sessions --json`
...
clawbot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:
- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- clawbot strips/suppresses this in the delivery layer.

As of `2026.1.10`, clawbot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
clawbot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but clawbot’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `clawbot status`.
