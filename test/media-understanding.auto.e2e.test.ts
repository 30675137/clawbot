...
import type { clawbotConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("clawbot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("clawbot-bin-sherpa-");
      const modelDir = await makeTempDir("clawbot-sherpa-model-");
      ...
      const cfg: clawbotConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("clawbot-bin-whispercpp-");
      const modelDir = await makeTempDir("clawbot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("clawbot-bin-gemini-");
      ...
      const cfg: clawbotConfig = { tools: { media: { image: {} } } };
