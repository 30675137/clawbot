...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["clawbotPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["clawbot", "pollId"]) ??
    readNestedString(value, ["clawbot", "poll", "id"]) ??
    readNestedString(value, ["data", "clawbotPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "clawbot", "pollId"]);
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          clawbotPollId: pollId,
        },
        msteams: {
          type: "messageBack",
          text: "clawbot poll vote",
          displayText: "Vote recorded",
          value: { clawbotPollId: pollId },
        },
      },
    ],
  };
