...
function resolveclawbotRoot(): string {
  if (coreRootCache) return coreRootCache;
  const override = process.env.clawbot_ROOT?.trim() || process.env.CLAWDBOT_ROOT?.trim();
  if (override) {
  ...
  for (const start of candidates) {
    for (const name of ["clawbot", "clawbot"]) {
  ...
  throw new Error(
    "Unable to resolve core root. Set clawbot_ROOT (or legacy CLAWDBOT_ROOT) to the package root.",
  );
}

async function importCoreModule<T>(relativePath: string): Promise<T> {
  const root = resolveclawbotRoot();
