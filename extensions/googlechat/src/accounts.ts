...
export function listGoogleChatAccountIds(cfg: clawbotConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultGoogleChatAccountId(cfg: clawbotConfig): string {
...
export function listEnabledGoogleChatAccounts(cfg: clawbotConfig): ResolvedGoogleChatAccount[] {
