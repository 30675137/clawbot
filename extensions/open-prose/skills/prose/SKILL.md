...
metadata: {"clawbot":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == clawbot `sessions_spawn`
- **File I/O** == clawbot `read`/`write`
- **Remote fetch** == clawbot `web_fetch` (or `exec` with curl when POST is required)
