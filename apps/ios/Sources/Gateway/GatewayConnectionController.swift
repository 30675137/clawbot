...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [clawbotCapability.canvas.rawValue, clawbotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(clawbotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(clawbotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = clawbotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(clawbotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            clawbotSystemCommand.notify.rawValue,
            clawbotSystemCommand.which.rawValue,
            clawbotSystemCommand.run.rawValue,
            clawbotSystemCommand.execApprovalsGet.rawValue,
            clawbotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(clawbotCapability.camera.rawValue) {
            commands.append(clawbotCameraCommand.list.rawValue)
            commands.append(clawbotCameraCommand.snap.rawValue)
            commands.append(clawbotCameraCommand.clip.rawValue)
        }
        if caps.contains(clawbotCapability.location.rawValue) {
            commands.append(clawbotLocationCommand.get.rawValue)
        }
