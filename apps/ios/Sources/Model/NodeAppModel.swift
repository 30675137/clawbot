...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = clawbotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = clawbotCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = clawbotCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = clawbotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = clawbotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__clawbot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: clawbotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for clawbot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case clawbotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case clawbotCanvasCommand.present.rawValue,
                 clawbotCanvasCommand.hide.rawValue,
                 clawbotCanvasCommand.navigate.rawValue,
                 clawbotCanvasCommand.evalJS.rawValue,
                 clawbotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case clawbotCanvasA2UICommand.reset.rawValue,
                 clawbotCanvasA2UICommand.push.rawValue,
                 clawbotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case clawbotCameraCommand.list.rawValue,
                 clawbotCameraCommand.snap.rawValue,
                 clawbotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case clawbotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: clawbotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: clawbotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(clawbotLocationGetParams.self, from: req.paramsJSON)) ??
            clawbotLocationGetParams()
        ...
        let payload = clawbotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case clawbotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(clawbotCanvasPresentParams.self, from: req.paramsJSON)) ??
                clawbotCanvasPresentParams()
        ...
        case clawbotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case clawbotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(clawbotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case clawbotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(clawbotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case clawbotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(clawbotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case clawbotCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              if (!globalThis.clawdbotA2UI) return JSON.stringify({ ok: false, error: "missing clawbotA2UI" });
              return JSON.stringify(globalThis.clawdbotA2UI.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case clawbotCanvasA2UICommand.push.rawValue, clawbotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == clawbotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(clawbotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try clawbotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(clawbotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try clawbotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                if (!globalThis.clawdbotA2UI) return JSON.stringify({ ok: false, error: "missing clawbotA2UI" });
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case clawbotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case clawbotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(clawbotCameraSnapParams.self, from: req.paramsJSON)) ??
                clawbotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case clawbotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(clawbotCameraClipParams.self, from: req.paramsJSON)) ??
                clawbotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(clawbotScreenRecordParams.self, from: req.paramsJSON)) ??
            clawbotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> clawbotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return clawbotLocationMode(rawValue: raw) ?? .off
    }
