...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/clawbot/canvas/<session>/`).
...
public let controlSocketPath = FileManager()
    ...
    .appendingPathComponent("Library/Application Support/clawbot/control.sock")
    .path
