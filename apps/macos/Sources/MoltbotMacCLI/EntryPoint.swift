...
struct clawbotMacCLI {
    static func main() async {
        ...
        default:
            fputs("clawbot-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      clawbot-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      clawbot-mac discover [--timeout <ms>] [--json] [--include-local]
      clawbot-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      clawbot-mac connect
      clawbot-mac connect --url ws://127.0.0.1:18789 --json
      clawbot-mac discover --timeout 3000 --json
      clawbot-mac wizard --mode local
    """)
}
