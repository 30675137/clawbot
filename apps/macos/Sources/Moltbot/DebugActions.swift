...
enum DebugActions {
    private static let verboseDefaultsKey = "clawbot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "clawbot.onboardingSeen"
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "clawbot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent(".clawdbot/clawbot.json")
        guard
