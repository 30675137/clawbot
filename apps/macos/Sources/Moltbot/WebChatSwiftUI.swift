...
struct MacGatewayChatTransport: clawbotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> clawbotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(clawbotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> clawbotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                clawbotGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<clawbotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any clawbotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = clawbotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: clawbotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<clawbotChatView>) -> NSViewController
    {
