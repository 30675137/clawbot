...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (clawbot token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: clawbotOAuthStore.AnthropicOAuthStatus = clawbotOAuthStore
...
enum clawbotOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let clawbotOAuthDirEnv = "CLAWDBOT_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "clawbot OAuth token file not found"
            case .unreadableFile: "clawbot OAuth token file not readable"
            case .invalidJSON: "clawbot OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in clawbot OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "clawbot OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.clawbotOAuthDirEnv]?
