...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: clawbotOAuthStore.AnthropicOAuthStatus = clawbotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: clawbotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = clawbotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = clawbotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try clawbotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. clawbot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: clawbotOAuthStore.AnthropicOAuthStatus,
