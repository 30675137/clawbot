...
enum GatewayEnvironment {
    ...
    static func gatewayPort() -> Int {
        ...
        if let configPort = clawbotConfigFile.gatewayPort(), configPort > 0 {
            return configPort
        }
    ...
    static func check() -> GatewayEnvironmentStatus {
        ...
        case let .success(runtime):
            let gatewayBin = CommandResolver.clawbotExecutable()
            ...
            if gatewayBin == nil, projectEntrypoint == nil {
                return GatewayEnvironmentStatus(
                    ...
                    gatewayVersion: nil,
                    requiredGateway: expectedString,
                    message: "clawbot CLI not found in PATH; install the CLI.")
            }
    ...
    static func installGlobal(versionString: String?, statusHandler: @escaping @Sendable (String) -> Void) async {
        ...
        let (label, cmd): (String, [String]) =
            if let npm {
                ("npm", [npm, "install", "-g", "clawbot@\(target)"])
            } else if let pnpm {
                ("pnpm", [pnpm, "add", "-g", "clawbot@\(target)"])
            } else if let bun {
                ("bun", [bun, "add", "-g", "clawbot@\(target)"])
            } else {
                ("npm", ["npm", "install", "-g", "clawbot@\(target)"])
            }

        statusHandler("Installing clawbot@\(target) via \(label)…")
        ...
        if response.success {
            statusHandler("Installed clawbot@\(target)")
        } else {
