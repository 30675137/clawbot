...
actor GatewayEndpointStore {
    ...
    init(deps: Deps = .live) {
        ...
        if let modeRaw {
            ...
            let seen = UserDefaults.standard.bool(forKey: "clawbot.onboardingSeen")
            initialMode = seen ? .local : .unconfigured
        }

        let port = deps.localPort()
        let bind = GatewayEndpointStore.resolveGatewayBindMode(
            root: clawbotConfigFile.loadDict(),
            env: ProcessInfo.processInfo.environment)
        let customBindHost = GatewayEndpointStore.resolveGatewayCustomBindHost(root: clawbotConfigFile.loadDict())
