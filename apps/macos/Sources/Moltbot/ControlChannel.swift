...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: clawbotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: clawbotKit.AnyCodable]()) {
                $0[$1.key] = clawbotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: clawbotProtocol.AnyCodable?) -> [String: clawbotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: clawbotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: clawbotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: clawbotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("clawbot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("clawbot.control.agent")
}
