...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("clawbot").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing clawbot CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed clawbot \($0)." } ?? "Installed clawbot."
