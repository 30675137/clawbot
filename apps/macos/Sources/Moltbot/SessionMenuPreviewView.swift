...
enum SessionMenuPreviewLoader {
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> clawbotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: clawbotSessionPreviewEntry,
    ...
    private static func cache(payload: clawbotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: clawbotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [clawbotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [clawbotKit.AnyCodable]) -> [clawbotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(clawbotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: clawbotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: clawbotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: clawbotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: clawbotChatMessage) -> String? {
