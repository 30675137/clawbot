...
enum LogLocator {
    private static var logDir: URL {
        ...
        return URL(fileURLWithPath: "/tmp/clawbot")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("clawbot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("clawbot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/clawbot/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("clawbot") && $0.pathExtension == "log" }
            .max { lhs, rhs in
