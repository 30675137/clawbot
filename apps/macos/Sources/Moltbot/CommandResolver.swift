...
enum CommandResolver {
    private static let projectRootDefaultsKey = "clawbot.gatewayProjectRootPath"
    private static let helperName = "clawbot"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/clawbot.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/clawbot")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let clawbotPaths = self.clawbotManagedPaths(home: home)
        if !clawbotPaths.isEmpty {
            extras.insert(contentsOf: clawbotPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + clawbotPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func clawbotManagedPaths(home: URL) -> [String] {
    ...
    static func clawbotExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectclawbotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/clawbot.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyclawbotInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.clawbotExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func clawbotNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let clawbotPath = self.projectclawbotExecutable(projectRoot: root) {
                return [clawbotPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "clawbot", subcommand] + extraArgs
            }
            if let clawbotPath = self.clawbotExecutable(searchPaths: searchPaths) {
                return [clawbotPath, subcommand] + extraArgs
            }

            let missingEntry = """
            clawbot entrypoint missing (looked for dist/index.js or bin/clawbot.js); run pnpm build.
            """
    ...
    // Existing callers still refer to clawbotCommand; keep it as node alias.
    static func clawbotCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real clawbot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/clawbot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v clawbot >/dev/null 2>&1; then
          CLI="$(command -v clawbot)"
          clawbot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/clawbot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/clawbot.js"
            node "$PRJ/bin/clawbot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent clawbot"
          pnpm --silent clawbot \(quotedArgs);
        else
          echo "clawbot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? clawbotConfigFile.loadDict()
