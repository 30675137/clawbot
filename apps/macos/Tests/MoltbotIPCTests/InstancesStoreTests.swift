...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: clawbotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: clawbotProtocol.AnyCodable] = [
            "presence": .init([clawbotProtocol.AnyCodable(entry)]),
        ]
        let payload = clawbotProtocol.AnyCodable(payloadMap)
