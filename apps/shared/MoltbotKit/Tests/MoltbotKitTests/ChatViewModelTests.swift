...
private final class TestChatTransport: @unchecked Sendable, clawbotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [clawbotChatHistoryPayload]
    private let sessionsResponses: [clawbotChatSessionsListResponse]

    private let stream: AsyncStream<clawbotChatTransportEvent>
    private let continuation: AsyncStream<clawbotChatTransportEvent>.Continuation

    init(
        historyResponses: [clawbotChatHistoryPayload],
        sessionsResponses: [clawbotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<clawbotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> clawbotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? clawbotChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [clawbotChatAttachmentPayload]) async throws -> clawbotChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return clawbotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> clawbotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? clawbotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: clawbotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = clawbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = clawbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { clawbotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = clawbotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = clawbotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = clawbotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { clawbotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
