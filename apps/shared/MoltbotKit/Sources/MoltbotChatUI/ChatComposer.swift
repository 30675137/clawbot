...
@MainActor
struct clawbotChatComposer: View {
    @Bindable var viewModel: clawbotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(clawbotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(clawbotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \clawbotPendingAttachment.id)
                { (att: clawbotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            clawbotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(clawbotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(clawbotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(clawbotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
