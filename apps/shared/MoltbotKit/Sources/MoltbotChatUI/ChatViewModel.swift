...
private let chatUILogger = Logger(subsystem: "bot.molt", category: "clawbotChatUI")

@MainActor
@Observable
public final class clawbotChatViewModel {
    public private(set) var messages: [clawbotChatMessage] = []
    ...
    public var attachments: [clawbotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [clawbotChatPendingToolCall] = []
    public private(set) var sessions: [clawbotChatSessionEntry] = []
    private let transport: any clawbotChatTransport
    ...
    private var pendingToolCallsById: [String: clawbotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any clawbotChatTransport) {
    ...
    public var sessionChoices: [clawbotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [clawbotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [clawbotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: clawbotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [clawbotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: clawbotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [clawbotChatMessage]) -> [clawbotChatMessage] {
        var result: [clawbotChatMessage] = []
    ...
    private static func dedupeKey(for message: clawbotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [clawbotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> clawbotChatAttachmentPayload in
            clawbotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> clawbotChatSessionEntry {
        clawbotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: clawbotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: clawbotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: clawbotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = clawbotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> clawbotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
