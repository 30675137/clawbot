...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/clawbot/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "clawbot Re-Auth" -c "After getting key, run: ssh $SERVER '~/clawbot/scripts/mobile-reauth.sh'" --id clawd-auth
        ;;
    *)
