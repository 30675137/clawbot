#!/bin/bash
# Setup clawbot Auth Management System
...
echo "=== clawbot Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'clawbot-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  clawbot message: Send warning via clawbot itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/clawbot-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/clawbot-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/clawbot-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now clawbot-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status clawbot-auth-monitor.timer"
echo ""
echo "Quick commands:"
