...
# Build and bundle clawbot into a minimal .app we can open.
# Outputs to dist/clawbot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/clawbot.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/clawbot/clawbot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/clawbot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :clawbotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :clawbotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/clawbot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/clawbot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/clawbot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/clawbot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/clawbot/Resources/clawbot.icns" "$APP_ROOT/Contents/Resources/clawbot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/clawbot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying clawbotKit resources"
clawbotKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/clawbotKit_clawbotKit.bundle"
if [ -d "$clawbotKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/clawbotKit_clawbotKit.bundle"
  cp -R "$clawbotKIT_BUNDLE" "$APP_ROOT/Contents/Resources/clawbotKit_clawbotKit.bundle"
else
  echo "WARN: clawbotKit resource bundle not found at $clawbotKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running clawbot"
killall -q clawbot 2>/dev/null || true
