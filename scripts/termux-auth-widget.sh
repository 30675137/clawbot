...
# clawbot Auth Widget for Termux
...
# Check auth status
termux-toast "Checking clawbot auth..."

STATUS=$(ssh "$SERVER" '$HOME/clawbot/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/clawbot/scripts/claude-auth-status.sh json' 2>&1)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/clawbot/scripts/mobile-reauth.sh'"
                ;;
            *)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: clawbot won't work until re-auth"
                ;;
        esac
        ;;
