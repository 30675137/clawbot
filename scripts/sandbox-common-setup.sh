...
BASE_IMAGE="${BASE_IMAGE:-clawbot-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-clawbot-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=clawbot.sandbox=1)
NOTE
