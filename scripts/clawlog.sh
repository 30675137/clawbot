...
show_usage() {
    cat << EOF
clawlog - clawbot Logging Utility

USAGE:
    clawlog [OPTIONS]

DESCRIPTION:
    View clawbot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    clawbot logs flow through the macOS unified log (subsystem: bot.molt).
