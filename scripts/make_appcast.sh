...
ZIP=${1:?"Usage: $0 clawbot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/clawbot/clawbot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^clawbot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/clawbot/clawbot/releases/download/v${VERSION}/"}
