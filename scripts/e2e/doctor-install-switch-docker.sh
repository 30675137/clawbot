...
IMAGE_NAME="clawbot-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/clawbot-bin:$PATH"
  mkdir -p /tmp/clawbot-bin

  cat > /tmp/clawbot-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/clawbot-bin/systemctl

  cat > /tmp/clawbot-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/clawbot-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/clawbot"
  npm_entry="/tmp/npm-prefix/lib/node_modules/clawbot/clawbot.mjs"
  git_entry="/app/clawbot.mjs"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/clawbot-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/clawbot-gateway.service"
    if [ ! -f "$unit_path" ]; then
