...
IMAGE_NAME="clawbot-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/clawbot-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/clawbot-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/clawbot-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/clawbot-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.clawdbot/clawbot.json"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.clawdbot/clawbot.json" <<'"'"'JSON'"'"'
{
