...
This document describes how the clawbot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`clawbot acp` exposes an ACP agent over stdio and forwards prompts to a running
clawbot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a clawbot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `clawbot acp` over stdio.

Example config:

```bash
clawbot config set gateway.remote.url wss://gateway-host:18789
clawbot config set gateway.remote.token <token>
```

Example run:

```bash
clawbot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
clawbot acp --session agent:main:main
clawbot acp --session agent:design:main
clawbot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “clawbot ACP” to start a thread.

## Execution Model

- ACP client spawns `clawbot acp` and speaks ACP messages over stdio.
...
```bash
clawbot acp --session agent:main:main
clawbot acp --session-label "support inbox"
clawbot acp --reset-session
```
...
`clawbot acp` resolves the Gateway URL and auth from CLI flags or config:
